...
@ManagedBean
@ViewScoped
public class AsignacionesBean extends BotonesBean implements Serializable{
    ...
    public void aceptar(ActionEvent evento) {
        try {
            if (super.getEnRegistro()) {
                ...
                for(int i=0;i<this.ccolss.size();i++){
                this.respcol.setCodColaborador(this.registroResponsabilidadServicio.findbyCod(this.ccolss.get(i)));
